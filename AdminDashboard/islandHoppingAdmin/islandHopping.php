...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
                                ...
                                <img class="img-profile rounded-circle" src="../profileAdmin/image/<?php echo $image?>">
                            </a>
                      ...
                      <div class="card-body">
                        ...
                        <div class="table-responsive">
                          <table class="table table-hover">
                            <thead>
                                <tr>
                                    <a href="add_hopping.php"><button style="size: 20px; border-radius: 10px; width: 200px; height:40px; background-color: rgb(230, 144, 15); color: aliceblue;">Add Island Hopping</button></a>
                                </tr>
                                <br><br><tr>
                                <th>No.</th>
                                <th>Name</th>
            ...
            ?>
  <tr>
    <td data-th="No."><?php echo $rows['hopping_ID'];?></td>
    <td data-th="Name"><?php echo $rows['name'];?></td>
    <td data-th="Price(Adult)"><?php echo $rows['priceA']?></td>
    <td data-th="Price(Children)"><?php echo $rows['priceC']?></td>
    ...
    <td data-th="Address"><?php echo $rows['islandIncluded'];?></td>
    ...
    <td data-th="2nd PhoneNo"><?php echo $rows['itenerary'];?></td>

    <form action="update_hopping.php" method="post">
        <input type="hidden" name="id" value="<?php echo $rows['hopping_ID'];?>">
    ...
    <form action="delete.php" method="post">
      <input type="hidden" name="id" id="id" value="<?php echo $rows['hopping_ID'];?>">
          ...
          if(move_uploaded_file($_FILES["image2"]["tmp_name"], $targetFilePath2)){
              ...
              if(move_uploaded_file($_FILES["image3"]["tmp_name"], $targetFilePath3)){
                    ...
                    if(move_uploaded_file($_FILES["image4"]["tmp_name"], $targetFilePath4)){
                                ...
                                $sql="INSERT INTO `islandhopping`(`name`, `priceA`, `priceC`, `image1`, `image2`, `image3`, `image4`, `islandIncluded`, `packageIncluded`, `phonenum1`, `phonenum2`, `itenerary`) VALUES ('$name','$priceA','$priceC','$fileName','$fileName2','$fileName3','$fileName4','$island','$package','$phonenum1','$phonenum2','$itenerary')";
                                $db=$conn->query($sql);
...
if(isset($_POST['btnUpdate']) && !empty($_FILES["Updateimage1"]["name"]) && !empty($_FILES["Updateimage2"]["name"]) && !empty($_FILES["Updateimage3"]["name"]) && !empty($_FILES["Updateimage4"]["name"])){
    ...
    $priceA = $_POST['UpdatepriceA'];
    $priceC = $_POST['UpdatepriceC'];
    $island = $_POST['Updateisland'];
    $package = $_POST['Updatepackage'];
    $phonenum1 = $_POST['Updatephonenum1'];   
    $phonenum2 = $_POST['Updatephonenum2'];   
    $itenerary = $_POST['Updateitenerary']; 
          ...
          if(move_uploaded_file($_FILES["Updateimage2"]["tmp_name"], $targetFilePath2)){
              ...
              if(move_uploaded_file($_FILES["Updateimage3"]["tmp_name"], $targetFilePath3)){
                    ...
                    if(move_uploaded_file($_FILES["Updateimage4"]["tmp_name"], $targetFilePath4)){
                                ...
                                $sql="UPDATE `islandhopping` SET `name`='$name',`priceA`='$priceA', `priceC`='$priceC', `image1`='$fileName',`image2`='$fileName2',`image3`='$fileName3',`image4`='$fileName4',`islandIncluded`='$island',`packageIncluded`='$package',`phonenum1`='$phonenum1',`phonenum2`='$phonenum2' ,`itenerary`='$itenerary'WHERE `hopping_ID`='$id' ";
                                $db=$conn->query($sql);
...
if(isset($_POST['btnDeleteConfirm'])){
    // print_r("deleted");
      ...
      $sql="SELECT `image1`, `image2`, `image3`, `image4` FROM `islandhopping` WHERE hopping_ID='$Deleteid'";
      $result=$conn->query($sql);
      $rows=$result->fetch_assoc();
  
      $delete="DELETE FROM `islandhopping` WHERE hopping_ID='$Deleteid' ";
      $db=$conn->query($delete);
      if($db){
        echo "<script> alert('The file has been deleted successfully.'); </script>";
      }
