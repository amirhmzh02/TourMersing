...
$paymentsql="UPDATE `payment` SET `reference_no`='$refno',`payment_status`='$status' WHERE `billcode`='$billcode' " ;
...
$sql = "SELECT customer.fullname,customer.phone_number,customer.email,booking.destination,booking.checkIn_date,booking.checkOut_date,booking.number_of_pax,booking.total_price,payment.payment_id,payment.reference_no,payment.payment_status,payment.billcode FROM customer LEFT JOIN booking ON customer.customer_id = booking.customer_id LEFT JOIN payment ON payment.booking_id = booking.booking_id WHERE payment.billcode = '$billcode' " ;
$db = $conn -> query($sql);
...
$query = "INSERT INTO `receipt`(`fullname`, `phone`, `email`, `destination`, `checkIn`, `checkOut`, `pax`, `price`, `paymentID`) 
VALUES ('$fullname','$phone_number','$email','$destination','$checkInDate','$checkOutDate','$number_of_pax','$total_price','$payment_id')";



    $sqlreceipt = "INSERT INTO `receipt`( `payment_id`, `billcode`, `status`, `fullname`, `destination`, `checkIn`, `checkOut`, `pax`, `totalPrice`) 
    VALUES ('$payment_id' , '$billcode' , '$status' , '$fullname' , '$destination' , '$checkInDate' , '$checkOutDate' , '$number_of_pax' , '$total_price')";

    $resultreceipt = $conn->query($sqlreceipt);
    ...
    <div class="container">
        <div class="row">
            <div class="col-12">
                <nav class="main-nav">
                    <!-- ***** Logo Start ***** -->
                    <a href="index.html" class="logo" style="margin-top: 0px;">
                        <img src="assets/images/img-01.png" alt="">
                    </a>
                    <!-- ***** Logo End ***** -->
                    <!-- ***** Menu Start ***** -->
                    <ul class="nav">
                      <li><a href="https://etourmersing.com/index.php" >Go back</a></li>
...
<tr>
<td width="80%">
<h1 class="sm-text-lg all-font-roboto" style="font-weight: 700; line-height: 100%; margin: 0; margin-bottom: 4px; font-size: 24px;">Customer Receipt-VACATION</h1>
...
<table class="sm-leading-32" style="line-height: 28px; font-size: 14px;" width="100%" cellpadding="0" cellspacing="0" role="presentation">
    <tr>
        ...
        <td class="sm-inline-2" style="color: #080808; font-weight: 600; text-align: right;" width="50%" align="right"><?php echo $refno ?></td>
        <!-- <?php echo $refno ?> -->
    </tr>
    ...
    <tr>
      ...
      <td class="sm-w-3-4 sm-inline-block" style="color: #080808; font-weight: 600; text-align: right;" width="50%" align="right"><?php echo $destination ?></td>
    </tr>
</table>
<table width="100%" cellpadding="0" cellspacing="0" role="presentation">
<tr>
...
<p class="all-font-roboto" style="margin: 0; margin-bottom: 4px; color: #a0aec0; font-size: 10px; text-transform: uppercase; letter-spacing: 1px;">Check-in - Check-Out</p>
<p class="all-font-roboto" style="font-weight: 600; margin: 0; color: #000000;"><?php echo $checkInDate."--".$checkOutDate ?></p>
</td>
</tr>
</table>
<table width="100%" cellpadding="0" cellspacing="0" role="presentation">
...
<tr>
<td style="color: #718096;" width="50%">Pax</td>
<td style="color: #54c6c4; font-weight: 600; text-align: right;" width="50%" align="right"><?php echo $number_of_pax ?></td>
</tr>
<tr>
<td style="color: #718096;" width="50%">Total price</td>
<td style=" color: #54c6c4; font-weight: 600; text-align: right;" width="50%" align="right">RM<?php echo $total_price ?></td>
</tr>
<tr>
<td style="font-weight: 600; padding-top: 32px; color: #000000; font-size: 20px;" width="50%">Total</td>
<td style="font-weight: 600; padding-top: 32px; text-align: right; color: #68d391; font-size: 20px;" width="50%" align="right">RM<?php echo $total_price ?></td>
